...
\large Índices Afya de Qualidade de Vida: MedQoL (médicos) e IQoL (estudantes de medicina)}
\author{Afya}
\date{\today}
...
escoramento de dois instrumentos de qualidade de vida: o \textbf{Afya MedQoL}
(médicos formados, 13 itens, calibração 2024\_2) e o \textbf{IQoL} (estudantes
...
\begin{itemize}
    \item O \textbf{Afya MedQoL} assume \textbf{três domínios ortogonais e
    independentes} ($F_1$, $F_2$, $F_3$), cada um escorado por um subconjunto
    de itens que carrega em um único fator (estrutura simples).
    \item O \textbf{IQoL} assume uma estrutura \textbf{bifatorial}: cada item
...
\subsection{Afya MedQoL --- Médicos formados}
\label{sec:medqol}
...
\subsection{IQoL --- Estudantes de medicina}
\label{sec:iqol}

\subsubsection{Estrutura do instrumento}

O IQoL (\emph{Student Quality of Life Index}) é composto por 8 itens
...
Diferentemente do MedQoL, o IQoL modela cada respondente por um vetor
...
O escore global do IQoL é o composto ponderado dos três fatores específicos,
\textbf{sem} inversão de sinal (nenhum domínio é invertido no IQoL):
...
Diferentemente do MedQoL, $\hat\theta_{\text{global}}$ do IQoL é um composto
...
limiares/interceptos, pesos e, no caso do IQoL, também $\mu_g/\sigma_g$) são
